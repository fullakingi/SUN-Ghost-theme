...
(function ($) {
    ...
    $(document).ready(function(){
        ...
        if(post_title_limit){
            $('.home-template .post-title, .author-template .post-title, .tag-template .post-title').succinct({
                size: post_title_limit //in characters
            });
        }
        ...
        //disable auto slide on touch devices
        if(Modernizr.touch){
        ...
        //if the tag page has no cover photo
        if($('.tag-template .no-image').length == 1) {
            $(".tag-template main").css("margin-top","65px");
        }
